...
class FileRepository:
    ...
    async def get_by_id_owned_by_user(self, file_id: int, owner_id: int) -> File | None:
        result = await self.session.execute(
            select(File).where(and_(File.id == file_id, File.owner_id == owner_id))
        )
        return result.scalar_one_or_none()
    ...
    async def is_attached_to_message(self, file_id: int) -> bool:
        result = await self.session.execute(
            select(MessageFile).where(MessageFile.file_id == file_id)
        )
